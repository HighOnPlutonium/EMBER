...
pub(crate) unsafe fn recreate_swapchain(
    ...
    let (swapchain,format,extent,sync) = PerSwapchain::create_swapchain(&per_window.window, per_window.surface, device, physical_device, ext_surface, ext_swapchain).unwrap(); // todo!
